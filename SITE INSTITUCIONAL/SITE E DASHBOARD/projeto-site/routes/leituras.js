...
router.get('/dadoCPU', (request, response) => {
    var sql = `select top(5) CpuMaximo, dataConsulta from dadosMaquinas order by id desc;`;
    db.query(sql, function (err, result) {
        if (err) throw err;
        response.json(result);
    });
...
router.get('/dadoMemoria', (request, response) => {
    var sql = `select top(5) RamUsada, dataConsulta from dadosMaquinas order by id desc;`;
    db.query(sql, function (err, result) {
        if (err) throw err;
        response.json(result);
    });
